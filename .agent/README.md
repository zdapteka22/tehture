# loop-guard 2.2 — стоп только по делу, без ложного рестарта

Остановка агента разрешена только если она **обоснована**.
Пустой `done_when` (вопрос, «и», «чё так долго») — **не** нарушение и **не** `no_reason`.
`no_reason` всегда `CONTINUE`, даже если в конфиге `on_unjustified: restart`.
...
Если все пункты `done_when` подтверждены — `DONE`, задача закрыта, лишних ходов нет.
`classify` / `enforce` всегда пишут `loops` и `restarts` в `loop-guard.state.json`. Лимит рестартов не глушит сторож.

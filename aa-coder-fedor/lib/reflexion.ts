...
export function verbalLesson(name: string, observation: string): string {
  ...
  if (/browser_click|click_kit/i.test(name) || /не сработал клик|не нашёл на странице|timed out|intercepts pointer/i.test(observation)) {
    return `Reflexion: клик «${clip}» не попал. Не останавливайся и не проси человека нажать. Вызови click_kit с action=heal, затем browser_click снова — кодер сменит набор (JS / мышь / Playwright) и запомнит, какой сработал.`;
  }

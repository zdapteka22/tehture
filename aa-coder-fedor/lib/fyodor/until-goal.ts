...
export const GOAL_NUDGE =
  "Стоп. Ты описал шаг словами вместо вызова инструмента. «Запускаю сейчас» — не работа. Сразу вызови инструмент. Один-два хода и слово «готово» — не конец. PowerShell на этом ПК выключен: установщик запускай через cmd / AA-Coder-Fedor-3.0-Setup.bat, не .ps1. Несколько browser_* кликов — не конец. Если клик упал ошибкой (not found / timeout) — click_kit(heal), потом browser_click снова. Если клик прошёл, а URL тот же или раскрылось меню — это аккордеон: не вызывай click_kit и не жми ту же кнопку, жми появившийся пункт или browser_press Enter. Сейчас вызови следующий инструмент: browser_snapshot / browser_click / browser_press / click_kit / browser_type / run_terminal_cmd / operator_use / pc_click / web_search / download_file / inspect_apk / open_on_pc. Если ищешь файл в интернете — web_search первым, не API первоисточника. 400/404 = не тот эндпоинт, максимум 2 попытки. Если Observation это JSON error_code / Access denied API — это не DENIED системы: смени параметры и вызови снова в этом ходе. Если в снимке есть строка access_token: — это полный токен, копируй её целиком и сразу вызывай API. Не проси пользователя вставить токен и не спрашивай «что видно». Для сообщества ВК цель = group_id или vk.com/club… На Windows это cmd.exe, не bash и не PowerShell.";

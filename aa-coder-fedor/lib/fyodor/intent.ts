...
export function looksLikeShortNudge(text: string): boolean {
  ...
  return /^(и+|ну+|да|ага|угу|ок|далее|дальше|давай|ещё|еще|чини|делай|поехали|го|ну что|и что|и чё|и че|ну чё|ну че|ну давай|опять стоишь)$/i.test(
    t,
  );
}
...
export function looksUnfinished(text: string): boolean {
  ...
  if (
    /(сейчас (сделаю|создам|напишу|открою|исправлю|запущу)|запускаю|реальный вызов|могу (сделать|создать|написать)|давай я |план:|шаги:|продолжить\?|хотите чтобы|если нужно, (я )?могу|не успел|осталось|ещё надо|I (will|can|shall) (create|write|open|fix)|let me (create|write|open)|here'?s (the )?plan)/i.test(
      raw,
    )
  ) {
    return true;
...
export function looksLikeNarratingWork(text: string): boolean {
  ...
  if (!raw.trim()) return false;
  if (
    /(сейчас (я )?(читаю|прочитаю|прочту|посмотрю|открою|сделаю|создам|напишу|запущу|проверю|вызову|починю|исправлю|кликну|печатаю|управляю)|запускаю|установщик|реальный вызов|ход\s*\d+\s*[—\-:.]|зачитался|подменял действие|имитировать работу|приложение запущено)/i.test(
      raw,
    )
  ) {
    return true;
